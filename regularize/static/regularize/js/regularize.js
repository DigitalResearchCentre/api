var env = {
  ...
  COLLATE_URL: 'http://localhost:8000/regularize/collate/',
};
...
function collate(witnesses, callback) {
  ...
  return $.when($.get(env.COLLATE_URL, data)).then(function(data){
    var tokens = {};
    _.each(data.table, function(row){
      _.each(row, function(cell){
        tokens[cell] = null;
      });
    });
    data.tokens = _.keys(tokens);
    return data;
  });
}
...
$(document).ready(function(){
  ...
  entity.witnesses(function(witness){
    collate(witness, function(data) {
      ...
      allAlign = { 
        alignments: ruleSet.ruleSet.alignments
      };
...
function sendRule(newRule)
{
  ...
  $.post("http://127.0.0.1:8000/regularize/postNewRule/",
...
{  
  ...
  $.ajax({
    url: "http://127.0.0.1:8000/regularize/postRecollate/",
    dataType: 'json',
    type: 'post',
    async: false,
    data: newWitnesses,
    ...
    $.ajax({
      url: "http://127.0.0.1:8000/regularize/sendRecollate/",
      dataType: 'json',
      async: false,
      success: function(data){
  ...
  if(alignOn)
    {
      ...
      $.ajax({
        url: "http://127.0.0.1:8000/regularize/changeAligns/",
        dataType: 'json',
        type: 'POST',
        async: false,
            ...
            if(deleteRule != null && deleteRule.checked)
              {
                ...
                // send changed rules to server
                $.ajax({
                  url: "http://127.0.0.1:8000/regularize/changeRules/",
                  dataType: 'json',
                  type: 'POST',
                  async: false,
  ...
  if(line == 1)
    {
    ...
    else if(line != "IR")
      {
        ...
        window.location.href = "http://127.0.0.1:8000/regularize/?urn="+ temp + "&username=" + userName + "&page=" + returnUrl; 
}
...
function getEntity(jdata)
{
  ...
  $.ajax({
    url: "http://127.0.0.1:8000/regularize/postEntity/",
    dataType: 'json',
    type: 'post',
    async: false,
    data: jdata,
    ...
    $.ajax({
      url: "http://127.0.0.1:8000/regularize/sendEntity/",
      dataType: 'json',
      async: false,
      success: function(data){
        //console.log(data)
...
function sendAlign(newAlign)
{
  ...
  for(var i in newAlign.alignments)
    ...
    $.post("http://127.0.0.1:8000/regularize/postNewAlign/",
...
{
  window.location.href = "http://127.0.0.1:8000/regularize/chooseRuleSetsInterface/?urn=" + urn + "&username=" + userName + "&page=" + returnUrl;
}

function chooseTexts()
{
  window.location.href = "http://127.0.0.1:8000/regularize/chooseTextsInterface/?urn=" + urn + "&username=" + userName + "&page=" + returnUrl;
}
